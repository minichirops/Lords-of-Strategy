...
void draw() {
  ...
  if (mousePressed) {
    lightAnchorX = 190 - mouseX;
    lightAnchorY = 190 - mouseY;
  }
  lightX = (lightX * 3 + lightAnchorX)/4 + (int)(randomGaussian()*random(-3,4));
  lightY = (lightY * 3 + lightAnchorY)/4 + (int)(randomGaussian()*random(-3,4));
  ...
  for (int y = 1; y < 379; y++) {
    for (int x = 1; x < 379; x++) {
      ...
      float mult = map(dist(x, y, 190 - lightX, 190 - lightY), 0, 460, 2, LOD_FADE);
      ...
      int intensity = lightMap[lx + ly * 380] + 32;
